...
```{r}
library(corrplot)
corr_matrix <- data_ex2 %>% select(c(lnpop, unempl, sp65, gdl, bac08, beltsc, beltpr, zerotol, alr)) %>% cor()
...
```{r}
data_ex2$elasticity_lnpop=poisson_extended$coefficients['lnpop']
data_ex2$elasticity_unempl=data_ex2$unempl*poisson_extended$coefficients['unempl']
...
```
```{r}
ols_plot_cooksd_chart(poisson_extended)
```
```{r}
plot(poisson_extended)
```
```{r}
plot(cooks.distance(poisson_extended), type="h")
```
```{r}
...
plot_dfbetas <- function(model){
  dfbts <- dfbetas(model)
  #par(mfrow=c(ncol(dfbetas)%/%2 + ncol(dfbetas)%%2,2))
  for (x in colnames(dfbetas)){
    plot(dfbetas[,x], ylab=x)
    abline(h = 4/sqrt(nrow(dfbetas)), col = "red")
    abline(h = -4/sqrt(nrow(dfbetas)), col="red")
    text(names(dfbetas[,x][abs(dfbetas[,x]) > 4/sqrt(nrow(dfbetas))]), dfbetas[,x][abs(dfbetas[,x]) > 4/sqrt(nrow(dfbetas))], labels=names(dfbetas[,x][abs(dfbetas[,x]) > 4/sqrt(nrow(dfbetas))]),  cex=0.9, font=2)
  }
}
...
dfbetas_infl <- rbind(
  medians,
  dfbetas %>% slice(812,811,807,80)
)
rownames(dfbetas_infl) <- c("median",812,811,807,80)
dfbetas_infl
